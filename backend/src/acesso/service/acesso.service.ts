import {Acesso, IUsuarioAcesso, Token} from "../interface/acesso.interface";
...
export class AcessoService {
    ...
    async login(dados: Acesso): Promise<Token> {
        ...
        if(!senhaCorreta) {
            ...
            await this.acessoDao.salvarUsuarioAcesso(usuarioAcesso)
            throw new ErroValidacao(["Credenciais inválidas"]);
        }
    ...
    async criarConta(dados: Acesso): Promise<IUsuarioAcesso> {
        ...
        const acessoParaSalvar: Acesso = {
            email: dados.email,
            senha: senhaCriptografa
        }

        return this.acessoDao.salvar(acessoParaSalvar)
    }
